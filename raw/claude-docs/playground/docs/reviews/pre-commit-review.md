...
fixed before the commit: a missing blank line that will trip ruff in sevdesk_ingest.py, a
...
### 1. `sevdesk_ingest.py` — missing blank line between `_sub_doc_ref` and `_BIZ_ID_RE`

**File:** `va-langgraph/eval/ingest/sevdesk_ingest.py`, lines 164–166
...
### 4. `sevdesk_ingest.py` — `_sub_doc_ref` missing blank line at end of function
...
`billy-mcp` and `hc-rag-agent`. If the ADK gateway ever needs postgres (e.g. if it gains
...
- **`sevdesk_ingest.py` regex ordering** — moving `_ANGLE_URL_RE` and its application
...
- **`sevdesk_ingest.py` `_CHAIN_RE` broadening** — removing the `Original` suffix from
...
- `prompt_injection.py` pattern #6 (`api_key`, `secret`, `password` in any key=value)
  is aggressive — a user asking "how do I set my API key in sevdesk?" will trigger it.
