...
Two parallel tracks to integrate the custom RAG pipeline into the playground, replacing the unused AWS Bedrock Knowledge Base call in `mcp_servers/product-a/`.
...
| **Track 1** | Replace `fetch_support_knowledge` (Bedrock) in product-a MCP with thin RAG tool | **Deferred** — product-a corpus (`product-a_raw`) does not exist yet |
...
|---|---|---|---|---|
| `corpus-a_raw` | vendor-a help center, FAQ, blog, Atlassian/Confluence | **Indexed** (April 18) | 598 total (268 help + 54 FAQ + 169 blog + 107 Atlassian) | JSONL (scraped by `rag-v1_scraper`) |
| `product-a_raw` | product-a accounting help docs | **Not yet available** — to be scraped/exported | TBD | TBD |

> Note: "corpus-a" is the internal name for the vendor-a support assistant corpus. The raw JSONL files are already in `help-support-rag-agent/data/rag-v1_scraper/output/v2/`.
...
data/
  raw/
    corpus-a_raw/          ← scraped_help.jsonl, scraped_faq.jsonl, scraped_blog.jsonl, scraped_atlassian.jsonl
    product-a_raw/          ← (not yet available)

  stores/
    vectordb/           ← dense embeddings (DuckDB; rag_chunks table with FLOAT[] embedding column)
      corpus-a.duckdb      ← already exists: data/vectorstore/rag_index.duckdb (6.8MB, April 18)
      product-a.duckdb      ← built when product-a_raw is ready
    metadb/             ← document-level bookkeeping + sentence snippets
      corpus-a_meta.db     ← maps to existing ingest_documents + ingest_snippets tables in DuckDB
      product-a_meta.db
    graphdb/            ← entity/relation graph for multi-hop queries (future)
      corpus-a/            ← Neo4j or Kuzu (see below)
      product-a/
```
...
>
> Migration step: extract `ingest_documents` + `ingest_snippets` into a separate `corpus-a_meta.db`; keep `rag_chunks` in `corpus-a.duckdb`. Or keep all three in one file for local dev — decision at migration time.
...
**Fix during P2:** Add an `ingest_runs` table to metadb capturing model name, strategy, params, and run timestamp. Ensures reproducibility when reindexing for product-a or re-embedding with a different model.
...
- Queries are in **German** — corpus is vendor-a (German accounting SaaS)
...
```
corpus-a_raw/ → parse → chunk → embed → write → corpus-a.duckdb + corpus-a_meta.db (+ future: corpus-a_graph/)
product-a_raw/ → parse → chunk → embed → write → product-a.duckdb + product-a_meta.db
```
...
### Goal
Migrate `help-support-rag-agent` into playground as a runnable peer service. Keep the full 9-node LangGraph graph (planner → retriever → confidence gates → reranker → answer → eval → summarizer). Use it for experimentation and calibration on the vendor-a (`corpus-a_raw`) corpus.

### Prerequisites
- [ ] Confirm `corpus-a_raw` sources and existing DuckDB index location in `help-support-rag-agent/data/`
...
**P2 — Ingest layer**
1. Create `va-support-rag/ingest/run_ingest.py` — CLI script that reads from `data/raw/corpus-a_raw/`, runs the existing preprocessing + chunking + embedding pipeline, writes to `data/stores/vectordb/corpus-a.duckdb` and `data/stores/metadb/corpus-a_meta.db`
2. Update `src/rag/datastore/duckdb.py` paths to read from `data/stores/vectordb/`
3. Document ingest command in `va-support-rag/Makefile` (`make ingest-corpus-a`)
...
3. Send one vendor-a support query via CLI runner; confirm answer + citations returned
...
1. Run eval suite against golden vendor-a dataset
...
## Track 1 — product-a MCP RAG tool (deferred)

**Blocked on:** `product-a_raw` corpus (product-a help docs not yet scraped/exported).

When unblocked:

1. Run ingest pipeline on `product-a_raw/` → `data/stores/vectordb/product-a.duckdb`
2. In `mcp_servers/product-a/app/tools/support_knowledge.py`:
...
3. Remove `boto3` from `mcp_servers/product-a/pyproject.toml`
4. Update `mcp_servers/product-a/.env.example`: remove `BEDROCK_KNOWLEDGE_BASE_ID`, `AWS_REGION`, `AWS_PROFILE`; add `VECTORDB_PATH`, `RERANKER_BACKEND`
5. Point at `data/stores/vectordb/product-a.duckdb` (shared volume with `va-support-rag`)
...
|---|---|---|
| Where is corpus-a_raw data? | P0 step 6 | **Resolved** — `data/rag-v1_scraper/output/v2/*.jsonl` |
...
| product-a help docs source? | Track 1 | **Open** — web scrape? Intercom export? Notion? |
...
Once both tracks are running against the same product-a corpus:
