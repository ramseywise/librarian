...
The same issue exists in `va-google-adk` for the email/invitation sub-agents once Billy test account is ready — but the immediate fix is the LangGraph domain.
...
**va-google-adk (future — apply when Billy test account is ready)**
