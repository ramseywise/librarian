...
> Scope: Containers + Terraform for va-google-adk, va-langgraph, product-a MCP, and frontend.
> Ignores: librarian stack entirely.
> Reference: va-agent-systems.md, va-agent-improvements.md, ts_google_adk (production reference on product-a-staging/va-agents-service)
...
MCP is strictly the tool layer (product-a data). The gateway calls `http://product-a-mcp:8765/sse` for tools, not for agent orchestration.
...
│      │ MCP/SSE                                               │
│  product-a-mcp :8765         FastMCP + SQLite product-a.db           │
│  product-a-api :8766         FastAPI REST stub (optional)        │
│  postgres  :5432         LangGraph checkpointer + product-aDB    │
└─────────────────────────────────────────────────────────────┘
```

Frontend env var `VA_BACKEND_URL` selects which gateway to target.
Both gateways share the same product-a MCP server and Postgres instance.
...
infrastructure/
  containers/
    ...
    product-a-mcp/
      Dockerfile            # copies adk-agent-pocs/mcp_servers/product-a/
    product-a-api/
      Dockerfile            # product-a REST stub (adk-agent-pocs/mcp_servers/product-a/main.py)
    frontend/
...
infrastructure_as_code/
  va_agents/
    ...
    environments/
      ...
      prod.tfvars
    modules/
      ecs_va/             # ECS cluster + task defs + services (gateway, product-a-mcp)
        main.tf
        variables.tf
        outputs.tf
      alb_va/             # ALB + target groups + listeners (reuses vpc from shared)
        ...
        variables.tf
        outputs.tf
      rds_va/             # RDS Postgres (LangGraph checkpointer + product-aDB prod)
        main.tf
        variables.tf
        outputs.tf
...
| ECS task def: `product-a-mcp` | product-a MCP + REST server, 256 CPU / 512MB |
...
| RDS Postgres (t3.micro dev, t3.small prod) | `va_product-a` DB (product-aDB prod) + `va_checkpoints` DB (LangGraph) |
| ECR repos | `va-gateway-adk`, `va-gateway-lg`, `product-a-mcp` |
| Secrets Manager | `GATEWAY_API_KEY`, `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`, `POSTGRES_URL` |
| CloudWatch log groups | one per ECS service |
| Security groups | ALB → gateway, gateway → product-a-mcp, gateway → RDS |
...
- [ ] `infrastructure/containers/product-a-mcp/Dockerfile`
- [ ] `infrastructure/containers/product-a-api/Dockerfile`
- [ ] `infrastructure/containers/docker-compose.yml` with postgres, both gateways, both product-a servers
...
- [ ] `modules/rds_va` — t3.micro, `va_product-a` + `va_checkpoints` databases
- [ ] Wire `POSTGRES_URL` secret → ECS task env
- [ ] va-langgraph: swap MemorySaver → AsyncPostgresSaver on startup
- [ ] product-a MCP: swap SQLite → Postgres for `product-a_BACKEND=postgres`
- [ ] Migration script in `product-a-mcp/` (`reset_db.py` → supports `--backend postgres`)
...
- [x] `docker-compose.va.yml` — frontend service added (port 3000, depends on product-a-mcp)
...
| 2 | product-aDB on Postgres in prod, or keep SQLite since it's a stub? | Phase 3 |
