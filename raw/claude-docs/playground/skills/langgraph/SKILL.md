...
description: >
  ...
  wiring MCP tools or adding a new domain to the intent router.
wiki:
...
**MCP backends:**
- `mcp_servers/knowledge_base/` — music KB tools (`search_articles`, `get_article`)
...
**MCP tools**
- Which MCP backend(s) does this use? Which specific tool functions?
...
**Testing**
- New domain → new fixtures in `tests/evalsuite/fixtures/support_tickets.json`?
